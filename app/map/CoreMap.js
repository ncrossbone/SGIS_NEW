Ext.define('Sgis.map.CoreMap', {
	...
	mapRendered: function(p){
        ...
        var customDistance = "";
        if(trident != null){
	        if(trident[1] == "6.0"){//IE 10버젼
	        	customDraw = "Sgis/map/toolbar/CustomDraw";
	        	customPrintTask = "Sgis/map/task/CustomPrintTask";
	        	customDistance = "Sgis/map/toolbar/CustomDistance";
	        }else if(trident[1] == "7.0"){//IE 11버젼
	        	customDraw = "Sgis/map/toolbar/CustomDraw";
	        	customPrintTask = "Sgis/map/task/CustomPrintTask";
	        	customDistance = "Sgis/map/toolbar/CustomDistance";
	        }else{//IE 그외 버젼
	...
	baseMapGray:function(mode){
		...
		if(Sgis.getApplication().browser=='Chrome' || Sgis.getApplication().browser=='Opera'){
			if(mode){
				...
				//document.getElementById("_mapDiv__layer0").style.background-color = "#94b7d4";
			}else{
				//console.info("else");
